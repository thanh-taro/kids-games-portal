...
- **New reward** → `js/rewards.js` `REWARDS[]` (alternates weapon→skill; there are no skins — the hero has one look). Skill rewards need a `skill:` id that exists in `SKILLS` plus a `color:` for the reward card; ensure `applyRewards` handles the type.
